<?php
// take-survey-24.php (Akademik Benlik Kavramı Ölçeği v6 - Seçenekler İfadelerle)
...
// Seçenekleri ifadelerle gösteriyoruz, değerleri harfler (A, B, C, D) olacak
$optionsMap = [
    "A" => "Hiçbir zaman",
    "B" => "Ara sıra",
    "C" => "Sık sık",
...
// Bu harita metin ifadesi -> sayısal puan (1-4) şeklinde olmalı
$textToScoreMap = [
    "Hiçbir zaman" => 1,
    "Ara sıra" => 2,
    "Sık sık" => 3,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $error === null) {
    ...
    // Cevaplar [question_db_id => answer_text (e.g., "A")] şeklinde geliyor
    ...
    if ($post_error === null) { // Veritabanı hatası yoksa devam et
        ...
        elseif (count($answers) < $totalExpectedQuestionsFetched) { // Çekilen soru sayısıyla karşılaştır
            ...
            // Cevapların geçerliliğini (optionsMap'in değerlerinden biri mi?) ve gönderilen ID'lerin DB'de varlığını kontrol et
    ...
    if ($post_error === null) { // Tüm doğrulamalar başarılıysa devam et
        if ($adminId !== null) {
            ...
            try {
                ...
                foreach ($answers as $actual_db_id => $answer_text) { // $answer_text artık "A", "B", "C", "D"
                    $qId_int = (int)$actual_db_id;
                    $answer_text_str = trim($answer_text); // "A", "B", "C", "D"
             ...
             // **** SENARYO 2: ADMIN YOK -> KAYDETME, SONUCU HESAPLA, SESSION'A AT, view-result-24.php'ye YÖNLENDİR ****
             ...
             if ($post_error === null && !empty($questionIdToSortOrderMap)) { // questionIdToSortOrderMap POST doğrulamada çekildi
                 // Alt ölçek madde listeleri (sort_order) - view-result-24.php'de tanımlanacak
                 // Session'a sadece cevapları [sort_order => answer_text] formatında kaydedelim
                 $sessionAnswersData = [];

                 foreach ($answers as $qDbId => $answerValue) { // $answerValue artık "A", "B", "C", "D"
                     ...
                     if ($sortOrder !== null) {
                         ...
                         // Skor hesaplaması view-result-24.php'de yapılacak.
                     } else {
                          error_log("Cannot find sort_order for QID: {$questionId} in survey {$surveyId} during Session handling");
                     }
                 }

                 // Session'a alt ölçek skorları ve toplam skor yerine sadece cevapları kaydediyoruz.
                 // Hesaplama view-result-24.php'de yapılacak.
                 ...
                 header('Location: ../admin/view-result-24.php');
                 exit();
            ...
            <form method="POST" id="surveyForm" action="<?= $actionUrl ?>" novalidate>
                ...
                <div class="info">
                 ...
                 <div class="instructions">
                     ...
                     <p>
                         <strong>A</strong> (Hiçbir zaman),
                         <strong>B</strong> (Ara sıra),
                         <strong>C</strong> (Sık sık),
                         <strong>D</strong> (Her zaman)
                     </p>
                 ...
                 ?>
                    <div class="question-block" id="q_block_<?= $question_id ?>">
                        ...
                        <div class="options-group-styled" role="radiogroup" aria-labelledby="q_text_<?= $question_id ?>">
                            <?php foreach ($optionsMap as $value => $label): // $value burada puanı temsil ediyor (1'den 4'e)
