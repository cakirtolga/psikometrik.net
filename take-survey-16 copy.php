    ...
    if ($post_error === null) {
        if ($adminId !== null) {
            ...
            try {
                ...
                // Cevapları Ekle (Gerçek Soru ID + Cevap Metni)
                ...
                foreach ($answers as $actual_db_id => $answer_score) {
                    ...
                    $answer_score_int = (int)$answer_score; // Puan 1-5
                    if (isset($optionsMap[$answer_score_int])) {
                        $answer_text_to_save = $optionsMap[$answer_score_int]; // Metni al
                        ...
                        // Bu durum normalde yaşanmamalı (yukarıda kontrol edildi)
                        error_log("Invalid score encountered during DB save: {$answer_score_int}");
                    }
                }
                $pdo->commit();
