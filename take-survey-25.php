<?php
// take-survey-25.php (Şiddet Algısı Anketi - Öğrenci)
...
$testTitleDefault = "Şiddet Algısı Anketi (ÖĞRENCİ)";
// ---------------------
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $error === null) {
    ...
    if ($post_error === null) { // Tüm doğrulamalar başarılıysa devam et
        if ($adminId !== null) {
            ...
            // **** SENARYO 2: ADMIN YOK -> KAYDETME, SONUÇ VERİSİNİ SESSION'A AT, view-result-25.php'ye YÖNLENDİR ****
            ...
            if ($post_error === null && !empty($questionIdToSortOrderMap)) { // questionIdToSortOrderMap POST doğrulamada çekildi
                ...
                header('Location: ../admin/view-result-25.php'); // view-result-25.php'ye yönlendir
                exit();
    ...
    <div class="container">
        ...
        <?php if (!$error && !empty($questions)): ?>
            ...
            <div class="instructions">
                <p>Bu bölümde bazı durumlar verilmiştir. Olayların karşısında yer alan "Şiddettir", "Kararsızım", "Şiddet Değildir" ifadelerinden size uygun seçeneğin altındaki parantezin içine (X) işareti koyunuz.</p>
         ...
         if (form) {
             form.addEventListener('submit', function(event) {
                 ...
                 if (!isValid) {
                     ...
                     // alert('Lütfen tüm gerekli alanları doldurun.'); // Daha detaylı hata mesajı HTML içinde gösteriliyor
