<?php
// take-survey-30.php (Öğrenci Rehberlik İhtiyacı Belirleme Anketi - RİBA Okul Öncesi Öğretmen)
...
// --- Konfigürasyon ---
$surveyId = 30; // Anket ID'si - Anket 30 için güncellendi
$testTitleDefault = "Öğrenci Rehberlik İhtiyacı Belirleme Anketi (RİBA) (Okul Öncesi-Öğretmen Formu)"; // Yeni anket başlığı
// ---------------------
...
// --- Sabit Veriler (Anket 30 - RİBA Okul Öncesi Öğretmen) ---
// Cevap Seçenekleri - Bunlar doğrudan kaydedilecek metinlerdir.
// Anket 30 PDF'ine göre güncellendi: "Hayır Kesinlikle Katılmıyorum", "Hayır Katılmıyorum", "Kararsızım", "Evet Katılıyorum", "Evet Kesinlikle Katılıyorum"
$optionsList = ["Hayır Kesinlikle Katılmıyorum", "Hayır Katılmıyorum", "Kararsızım", "Evet Katılıyorum", "Evet Kesinlikle Katılıyorum"];
// Bu anketin puanlama mantığı (1-5) view-result dosyası tarafından işlenecektir.

// --- Yönetici ID Kontrolü ---
// URL'de admin_id varsa, bunun geçerli bir admin olup olmadığını kontrol et.
// Bu anket öğretmen formu olsa da, yöneticinin öğretmenler adına doldurması senaryosu olabilir.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $error === null) {
    ...
    // Öğretmen anketine özel alanlar (Ad Soyad, Branş, Cinsiyet ve Diğer İhtiyaçlar eklendi)
    $teacherName = trim(htmlspecialchars($_POST['teacher_name'] ?? '', ENT_QUOTES, 'UTF-8')); // Ad Soyad alanı (Zorunlu yapıldı)
    $teacherBranch = trim(htmlspecialchars($_POST['teacher_branch'] ?? '', ENT_QUOTES, 'UTF-8')); // Branş alanı (Zorunlu yapıldı)
    $teacherGender = isset($_POST['teacher_gender']) ? trim(htmlspecialchars($_POST['teacher_gender'], ENT_QUOTES, 'UTF-8')) : null; // Cinsiyet alanı (Zorunlu)
    $otherNeeds = trim(htmlspecialchars($_POST['other_needs'] ?? '', ENT_QUOTES, 'UTF-8')); // Diğer ihtiyaçlar alanı


    // Cevaplar [question_db_id => answer_text (e.g., "Kararsızım")] şeklinde geliyor
    ...
    if ($post_error === null) { // Veritabanı hatası yoksa devam et
        ...
        } elseif (empty($teacherName)) { // Ad Soyad kontrolü (Zorunlu yapıldı)
    ...
    if ($post_error === null) { // Tüm doğrulamalar başarılıysa devam et
        if ($adminId !== null) {
            ...
            try {
                ...
                // survey_participants tablosunda 'name' öğretmen branşı, 'class' cinsiyet
                // 'description' sütununa Ad Soyad ve diğer ihtiyaçlar alanını kaydedeceğiz.
                $teacherInfoForName = !empty($teacherBranch) ? $teacherBranch : "Belirtilmemiş"; // Branş (name sütununa)
                $teacherInfoForClass = !empty($teacherGender) ? $teacherGender : "Belirtilmemiş"; // Cinsiyet (class sütununa)

                // Ad Soyad ve Diğer İhtiyaçları description sütununda birleştirerek kaydet
                $descriptionContent = "Ad Soyad: " . (!empty($teacherName) ? $teacherName : "Belirtilmemiş") . "\n";
                $descriptionContent .= "Diğer İhtiyaçlar: " . (!empty($otherNeeds) ? $otherNeeds : "Belirtilmemiş");
                ...
                $stmtParticipant->bindParam(':name', $teacherInfoForName, PDO::PARAM_STR); // Branş (name sütununa)
                $stmtParticipant->bindParam(':class', $teacherInfoForClass, PDO::PARAM_STR); // Cinsiyet (class sütununa)
                ...
                // Log mesajı öğretmen anketine göre güncellendi
                error_log("Admin Survey Submit Exception (S{$surveyId}, Admin{$adminId}, TeacherName:{$teacherName}, TeacherBranch:{$teacherBranch}, TeacherGender:{$teacherGender}, OtherNeeds:{$otherNeeds}): " . $e->getMessage());
                $post_error = "<b>Kayıt Sırasında Bir Hata Oluştu!</b> Lütfen tekrar deneyin veya yöneticiyle iletişime geçin. (Detaylar loglandı)";
            }

        } else {
            // **** SENARYO 2: ADMIN YOK -> KAYDETME, SONUÇ VERİSİNİ SESSION'A AT, view-result-30.php'ye YÖNLENDİR ****
            ...
            if ($post_error === null && !empty($questionIdToSortOrderMap)) { // questionIdToSortOrderMap POST doğrulamada çekildi
                ...
                foreach ($answers as $qDbId => $answerValue) { // $answerValue artık seçenek metinlerinden biri
                    ...
                    if ($sortOrder !== null) {
                        ...
                        // view-result-30.php sort_order üzerinden çalışıyor gibi görünüyor.
                ...
                // view-result-30.php'ye yönlendir
                header('Location: ../admin/view-result-30.php');
                exit();
...
<head>
    ...
    <meta name="description" content="<?= htmlspecialchars($pageTitle) ?> doldurma sayfası. Okul öncesi öğretmenleri için rehberlik ihtiyaçları belirleme anketi.">
    <meta name="keywords" content="rehberlik ihtiyaçları, okul öncesi, öğretmen anketi, RİBA, anket doldur, eğitim, psikoloji">
    ...
    <div class="container">
        ...
        <?php if (!$error && !empty($questions)): ?>
            ...
            <form method="POST" id="surveyForm" action="<?= $actionUrl ?>" novalidate>
                ...
                <?php // Öğretmen bilgileri (Ad Soyad, Branş ve Cinsiyet) ?>
                <div class="info">
                     <div class="info-grid">
                         <div>
                             <label for="teacher_name">Ad Soyadınız <span class="required-star">*</span></label> <?php // Zorunlu yapıldı ?>
                             <input type="text" id="teacher_name" name="teacher_name" required placeholder="Adınızı ve soyadınızı girin..." value="<?= htmlspecialchars($form_data['teacher_name'] ?? '') ?>" class="<?= ($post_error && empty($form_data['teacher_name'])) ? 'validation-error' : '' ?>" aria-describedby="teacherNameError">
                             <p id="teacherNameError" class="validation-error-text" aria-live="polite"></p>
                         </div>
                         <div>
                             <label for="teacher_branch">Branşınız <span class="required-star">*</span></label> <?php // Zorunlu yapıldı ?>
                ...
                <?php foreach ($questions as $question):
                    ...
                    $sort_order = $question['sort_order']; // Sorunun sıra numarası (1-37)
        ...
        // Eksik varsa alert gösterir ve formu GÖNDERMEZ.
         ...
         if (form) {
             form.addEventListener('submit', function(event) {
                 ...
                 const nameErrorP = document.getElementById('teacherNameError');
                 ...
                 // Branş kontrolü (Zorunlu yapıldı)
