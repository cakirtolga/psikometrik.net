...
// Düzeltme: config.php dosyasının doğru yolu ayarlandı
require_once __DIR__ . '/src/config.php'; // src klasörünün take-survey-7.php ile aynı dizinde olduğunu varsayar

// Define the survey ID for this specific file (Bu dosya için anket ID'si belirlenir)
$surveyId = 7; // Anket ID'si 7 olarak ayarlandı (Beck Anksiyete Testi)
    ...
    try {
        ...
        if ($admin) {
            $adminId = $admin['id']; // Admin bulundu, ID'yi kullanabiliriz
             error_log("Admin ID validated from URL for Survey 7: " . $adminId);
        } else {
            $error = 'Geçersiz veya bulunamayan yönetici ID\'si.';
            error_log($error . " - Attempted ID for Survey 7: " . $potentialAdminId);
             // adminId null kalır
        }
    } catch (PDOException $e) {
        $error = 'Yönetici bilgisi alınırken veritabanı hatası: ' . $e->getMessage();
        error_log("Admin ID validation PDO Exception for Survey 7: " . $e->getMessage());
         // adminId null kalır
    }
} else {
    $error = 'Yönetici ID\'si eksik veya geçersiz.';
    error_log($error . " - Received admin_id for Survey 7: " . ($_GET['admin_id'] ?? 'Not set'));
     // adminId null kalır
}
...
// Fetch survey information for Survey ID 7 (Anket bilgileri çekilir)
...
// Fetch questions for Survey ID 7 ordered by sort_order (Anketin soruları çekilir)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    error_log("--- POST request received for Survey 7 ---");
    ...
    error_log("POST Data Received for Survey 7: " . print_r($_POST, true));
    error_log("Name: " . $name);
    ...
    if (empty($name) || empty($class)) {
        ...
        error_log("Validation failed for Survey 7: Name or Class is empty.");
    }
    ...
    // This ensures all questions across all pages have been answered
         ...
         error_log("Validation failed for Survey 7: Incorrect number of answers received. Expected: " . $totalQuestions . ", Received: " . count($answers));
         ...
         if (!empty($missingQuestionIds)) {
             error_log("Missing question IDs for Survey 7: " . implode(', ', $missingQuestionIds));
         }
    ...
    // If validation passes, proceed with database insertion (Doğrulama başarılıysa veritabanına kayıt yapılır)
        ...
        if (is_null($adminId)) {
             ...
             error_log("Insertion blocked for Survey 7: Admin ID is null.");
        } else {
            try {
                // Start a transaction for atomic database operations (Veritabanı işlemleri için transaction başlatılır)
                $pdo->beginTransaction();
                error_log("Transaction started for Survey 7.");

                // --- Insert Participant --- (Katılımcı kaydı yapılır)
                error_log("Attempting to insert participant for Survey 7: Name=" . $name . ", Class=" . $class . ", SurveyId=" . $surveyId . ", AdminId=" . $adminId);
                ...
                if ($participantInsertSuccess) {
                     error_log("Participant inserted successfully for Survey 7 with ID: " . $participantId);
                } else {
                     error_log("Participant insert failed for Survey 7. PDO ErrorInfo: " . print_r($stmt->errorInfo(), true));
                ...
                error_log("Attempting to insert answers for Survey 7, participant ID: " . $participantId);
                ...
                foreach ($answers as $qid => $answer) {
                    ...
                    if (is_numeric($qid) && is_string($answer)) {
                        ...
                        if ($answerInsertSuccess) {
                             error_log("Inserted answer for Survey 7, QID " . $qid . ": " . $answer);
                        } else {
                             error_log("Answer insert failed for Survey 7, QID " . $qid . ". PDO ErrorInfo: " . print_r($stmt->errorInfo(), true));
                        ...
                        error_log("Skipping invalid answer data for Survey 7: QID=" . $qid . ", Answer=" . $answer);
                ...
                if ($participantInsertSuccess && $allAnswersInserted) { // Katılımcı eklendiyse VE tüm cevaplar başarılıysa commit yapılır
                     $pdo->commit();
                     error_log("Transaction committed for Survey 7. Redirecting to tamamlandi.php");
                     ...
                     error_log("Transaction rolled back for Survey 7 due to insertion failure.");
                }
                ...
                error_log("PDO Exception for Survey 7: " . $e->getMessage());
                ...
                // $error = "Anket kaydedilirken bir hata oluştu.";
                 ...
                 error_log("General Exception for Survey 7: " . $e->getMessage()); // Hata loglanır
            }
        }
    }
                        ...
                        ?>
                            <div class="mb-8 question">
                                ...
                                <div class="flex gap-3 mb-6 options">
                                    <?php
                                    // Use the specific options for Survey 7: 'Hiç', 'Hafif', 'Orta', 'Ciddi'
                                    $options = ['Hiç', 'Hafif', 'Orta', 'Ciddi'];
