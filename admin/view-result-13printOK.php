<?php
// view-result-13.php (Beck Depresyon Ölçeği Sonuçları - Yazdırma Düzeltmeleri v7 - Logo URL Fix)
...
// --- Logo URL Tanımlamaları (!!! WEB URL'lerini KONTROL EDİN/GÜNCELLEYİN !!!) ---
// Bu yollar tarayıcının doğrudan erişebileceği URL'ler olmalı (genellikle alan adından sonraki kısım)
$institutionWebURL = null; // Kurum logosu için web URL'si (aşağıda doldurulacak)
$psikometrikWebURL = '/assets/Psikometrik.png'; // Psikometrik.net logosunun web URL'si (Başındaki / önemli!)
...
try {
    ...
    // Kurum Logosu Web URL'sini Ayarla
    $rawInstitutionPathFromDB = $participant['institution_logo_path']; // DB'den gelen yol (örn: uploads/logos/kurum.png)
    if (!empty($rawInstitutionPathFromDB)) {
        // DB'den gelen yolun zaten web erişimine uygun olduğunu varsayalım (örn: başında / yok)
        // Güvenlik için başlangıçtaki / ve .. kaldıralım
        $cleanRelativePath = ltrim(str_replace('..', '', $rawInstitutionPathFromDB), '/');
        // Sunucudaki fiziksel yolu oluştur (web kök dizinine göre)
        // __DIR__ genellikle bu dosyanın bulunduğu dizindir, web kökü farklı olabilir.
        // DOCUMENT_ROOT genellikle daha güvenilirdir veya sabit bir yol kullanın.
        $docRoot = $_SERVER['DOCUMENT_ROOT'] ?? realpath(__DIR__ . '/..'); // Web kök dizinini bulmaya çalış
        $fullServerPath = $docRoot . '/' . $cleanRelativePath;

        if (file_exists($fullServerPath)) {
             // Dosya varsa, web URL'sini oluştur (başında / olsun)
             $institutionWebURL = '/' . $cleanRelativePath;
        } else {
             error_log("Kurum logosu dosyası bulunamadı (Sunucu Yolu): " . $fullServerPath);
             // Web URL'si null kalacak
        }
    }
    $institutionName = $participant['institution_name'];

    // Psikometrik Logo Dosya Varlığını Kontrol Et (Sunucu yoluyla)
    ...
    if (!file_exists($fullPsikoServerPath)) {
        error_log("Psikometrik logo dosyası bulunamadı (Sunucu Yolu): " . $fullPsikoServerPath);
    ...
    // --- Beck Cevap/Puanlama (Önceki gibi - Tam Liste Eklenmeli) ---
    ...
    if (!empty($raw_answers)) {
        ...
        foreach ($questions as $index => $q) {
            $qid = $q['id'];
            $maddeNumber = $index + 1;
            $answerText = $answersByQid[$qid] ?? "Cevap Yok";
            $itemScore = null;
            $optionsForThisItem = $beck_options[$maddeNumber] ?? null;
...
<head>
    ...
    <style>
        ...
        /* --- YAZDIRMA STİLLERİ (Fixed Header ile) --- */
        @media print {
             /* Ekran öğelerini GİZLE */
             .screen-only { display: none !important; }

             /* Yazdırma yapısını GÖSTER */
             ...
             .print-header-fixed {
                 position: fixed; top: 10mm; left: 10mm; right: 10mm; height: 60px;
                 display: flex !important; justify-content: space-between !important; align-items: center !important;
                 z-index: 10;
                 /* background-color: white; */ /* İçeriğin arkadan görünmemesi için gerekebilir */
             }
             .logo-container-fixed { height: 100%; display: flex; align-items: center; flex: 0 0 180px; }
             .logo-container-fixed.left { justify-content: flex-start; }
             .logo-container-fixed.right { justify-content: flex-end; }
             ...
             /* Ana İçerik Alanı (Başlığın altına itmek için) */
             .print-main-content {
                 margin: 10mm; /* Kenar boşlukları */
                 padding-top: calc(10mm + 60px + 5mm); /* top_margin + header_height + extra_space */
             }
             ...
             .print-info-box p { margin: 1.5mm 0; font-size: 9pt; }
             .print-info-box strong { display: inline-block; min-width: 70px; font-weight: bold; }
             ...
             .print-score-box p { margin: 1.5mm 0; font-size: 9pt; }
             .print-score-box .score { font-weight: bold; font-size: 10pt; }
             .print-score-box .interpretation { font-style: italic; }
...
<div class="container screen-only">
    ...
    <div class="screen-score-info mb-8">
    ...
    <div class="screen-button-footer">
     ...
     <div class="print-header-fixed">
          <div class="logo-container-fixed left">
               <?php if (!empty($institutionWebURL)): // Web URL'si var mı kontrol et ?>
          ...
          <div class="logo-container-fixed right">
               <?php if (!empty($psikometrikWebURL)): // Web URL'si var mı kontrol et ?>
                   <img src="<?= htmlspecialchars($psikometrikWebURL) ?>" alt="Psikometrik.Net Logo">
