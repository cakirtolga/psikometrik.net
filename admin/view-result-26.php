<?php
// view-result-26.php (Şiddet Algısı Anketi - Veli Sonuçları)
...
if (isset($_GET['id'])) {
    ...
    if (!$participantId) {
        ...
        try {
            ...
            if (!$participantData) {
                ...
                error_log("Participant not found for view-result-26 (ID: {$participantId}, Survey: {$surveyId})");
            } else {
                ...
                if ($dataSource == 'db' && !empty($participantData['admin_id']) && !empty($participantData['institution_logo_path'])) {
                   ...
                   else { error_log("Kurum logosu dosyası bulunamadı (view-result-26): " . $fullServerPath); }
                }
            ...
            error_log("DB Error view-result-26 (ID: {$participantId}): " . $e->getMessage());
    ...
    $dataSource = 'session';
    // take-survey-26.php Session'a 'answers' ([sort_order => answer_text]), katılımcı bilgisini kaydediyor.
    $dimensionCounts = [];
    $participantData = null;
    ...
    if (isset($_SESSION['survey_result_data'], $_SESSION['survey_result_data']['survey_id']) && $_SESSION['survey_result_data']['survey_id'] == $surveyId) {
        ...
        if (isset($sessionData['answers'], $sessionData['parent_name']) && is_array($sessionData['answers'])) { // Veli anketine özel kontrol
            ...
            if ($processedAnswerCount < $totalExpectedQuestionsFetched) {
                 error_log("view-result-26 Session: Processed fewer valid answers ({$processedAnswerCount}) than expected ({$totalExpectedQuestionsFetched}) from session data.");
            ...
            error_log("Incomplete session data for survey 26: " . print_r($sessionData, true));
...
if ($psikometrikWebURL) {
    ...
    if (!file_exists($fullPsikoServerPath)) { $psikometrikWebURL = null; error_log("Psikometrik logo dosyası bulunamadı (view-result-26): " . $fullPsikometrikServerPath); }
}
...
<div class="container">
    ...
    <?php else: // Katılımcı verisi var ?>
        ...
        <div class="participant-info">
             ...
             <p><strong>Veli Ad Soyad:</strong> <?= htmlspecialchars($participantData['name']) ?></p> <?php // Veli adı ?>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        optionsList.forEach((option, index) => {
            ...
            let color;
            // Renkleri isteğe göre ayarla: Şiddettir kırmızı, Kararsızım turuncu, Şiddet Değildir yeşil
