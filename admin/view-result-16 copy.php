...
$questions = [];
$participantAnswers = []; // [question_id => answer_text/score]
...
if (isset($_GET['id'])) {
    ...
    if (!$participantId) {
        ...
        try {
            ...
            foreach($questions as $q) {
                ...
                $answerText = $participantAnswers[$question_id] ?? null;
                $itemScore = null;
            ...
            if ($missingAnswers || $answeredQuestionsCount < count($questions)) { $totalScore = null; }
    ...
    if (isset($_SESSION['survey_result_data']) && isset($_SESSION['survey_result_data']['survey_id']) && $_SESSION['survey_result_data']['survey_id'] == $surveyId) {
        ...
        $sessionAnswers = $sessionData['answers']; // [qid => score (1-5)]
        $participantData = [
        ...
        try {
            ...
            foreach($questions as $q) {
                 ...
                 $processedAnswersForTable[] = [
                     'madde' => $sort_order, 'question_text' => $question_text,
                     'answer_text' => $answerText, 'score' => $itemScore
                 ];
             }
             // Session'dan gelen skor geçerli mi kontrol edilebilir (örn. tüm sorular var mı)
             if (count($sessionAnswers) < count($questions)) {
...
if ($psikometrikWebURL) { /* ... (Önceki kodla aynı) ... */ }
