<?php
// view-result-30.php (Öğrenci Rehberlik İhtiyacı Belirleme Anketi - RİBA Okul Öncesi Öğretmen Sonuçları)
...
$testTitleDefault = "Öğrenci Rehberlik İhtiyacı Belirleme Anketi (RİBA) (Okul Öncesi-Öğretmen Formu)"; // Varsayılan anket başlığı
...
$otherNeedsContent = null; // Öğretmenin belirttiği diğer ihtiyaçlar


// --- Puanlama Anahtarı (Anket 30) ---
// Hayır Kesinlikle Katılmıyorum: 1, Hayır Katılmıyorum: 2, Kararsızım: 3, Evet Katılıyorum: 4, Evet Kesinlikle Katılıyorum: 5
$scoringKey = [
...
$developmentAreas = [
    ...
    'Eğitsel Rehberlik' => range(30, 37), // 37 soru var
];
    ...
    if (!$participantId) {
        ...
        try {
            ...
            if (!$participantData) {
                ...
                error_log("Participant not found in DB for view-result-30 (ID: {$participantId}, Survey: {$surveyId})");
            } else {
                ...
                if (!empty($participantData['admin_id']) && !empty($participantData['institution_logo_path'])) {
                   ...
                   if (file_exists($fullServerPath)) {
                       ...
                       error_log("Kurum logosu dosyası bulunamadı (view-result-30): " . $fullServerPath);
                   }
                }

                // Öğretmenin belirttiği diğer ihtiyaçlar (description sütunından)
                ...
                // Toplam beklenen soru sayısı (37)
                ...
                // Tüm sorular cevaplanmış mı control et (37 soru bekleniyor)
                if ($processedAnswerCount < $totalExpectedQuestionsFetched) {
                    ...
                    $evaluationText = "Bu anket sonuçları, okul öncesi öğrencilerinin rehberlik ihtiyacına yönelik öğretmen algısını yansıtmaktadır. ";
                     ...
                     if ($validDimensionScoresExist) {
                         // Puanlama 1-5 arası
                         if ($highestDimension !== null && $highestAverage > 3) { // Ortalama 3'ün üzerindeyse anlamlı bir ihtiyaç algısı var diyebiliriz (1-5 arası puanlama)
                             $evaluationText .= "Sonuçlara göre, öğretmen algısında öğrenciler için en çok ihtiyaç duyulan alan '{$highestDimension}' olarak öne çıkmaktadır. ";
            ...
            error_log("DB Error view-result-30 (ID: {$participantId}): " . $e->getMessage());
...
if ($dataSource === null) {
    ...
    if (isset($_SESSION['survey_result_data'], $_SESSION['survey_result_data']['survey_id']) && $_SESSION['survey_result_data']['survey_id'] == $surveyId) {
        ...
        // Gerekli anahtarların varlığını kontrol et (Öğretmen anketine özel kontrol)
        if (isset($sessionData['answers'], $sessionData['teacher_branch'], $sessionData['teacher_gender']) && is_array($sessionData['answers'])) {

            // Session verilerini participantData yapısına uygun hale getir
            $participantData = [
                'name' => $sessionData['teacher_branch'] ?? 'Belirtilmemiş', // Öğretmen branşı
                'class' => $sessionData['teacher_gender'], // Öğretmen cinsiyeti
            ...
            // Öğretmenin belirttiği diğer ihtiyaçlar (session'dan)
            ...
            if ($processedAnswerCount < $totalExpectedQuestionsFetched) {
                 error_log("view-result-30 Session: Processed fewer valid answers ({$processedAnswerCount}) than expected ({$totalExpectedQuestionsFetched}) from session data.");
            ...
            error_log("Incomplete session data for survey 30: " . print_r($sessionData, true));
...
if ($psikometrikWebURL) {
    ...
    if (!file_exists($fullPsikoServerPath)) { $psikometrikWebURL = null; error_log("Psikometrik logo dosyası bulunamadı (view-result-30): " . $fullPsikoServerPath); }
}
...
<head>
    ...
    <meta name="description" content="<?= htmlspecialchars($pageTitle) ?> sonuçlarınızı görüntüleyin. Okul öncesi öğretmenleri için rehberlik ihtiyaçları belirleme anketi sonuçları.">
    <meta name="keywords" content="rehberlik ihtiyaçları, okul öncesi, öğretmen anketi, RİBA, anket sonuçları, eğitim, psikoloji">
    ...
    <div class="logo-right">
        ...
        if ($psikometrikWebURL) {
             ...
             if (file_exists($fullPsikoServerPath)) {
                 ...
                 error_log("Psikometrik logo dosyası bulunamadı (view-result-30): " . $fullPsikoServerPath);
             }
        }
        ?>
    ...
    <?php else: // Katılımcı verisi var veya en azından boyut isimleri mevcut ?>
        ...
        <div class="participant-info">
             ...
             <?php if ($dataSource == 'session'): ?>
                 <p><strong>Branş:</strong> <?= htmlspecialchars($participantData['name'] ?? 'Belirtilmemiş') ?></p>
                 <p><strong>Cinsiyet:</strong> <?= htmlspecialchars($participantData['class'] ?? 'Belirtilmemiş') ?></p>
                 ...
                 <table class="score-table">
                     <thead>
                         <tr>
                             <th>Gelişim Alanı</th>
                             <th>Toplam Puan</th>
                             <th>Ortalama Puan (1-5)</th> <?php // Puan 1-5 olacak ?>
                         </tr>
                     </thead>
                     <tbody>
        ...
        <?php // Öğretmenin Belirttiği Diğer İhtiyaçlar Alanı ?>
        <?php if ($otherNeedsContent): ?>
             <div class="other-needs-section">
                 <h3>Öğretmenin Belirttiği Diğer İhtiyaçlar</h3>
                 ...
                 options: {
                     ...
                     scales: {
                         x: { // X ekseni şimdi ortalama puanlar
                             beginAtZero: true,
                             max: 5, // Maksimum puan 5 (1-5 arası puanlama)
                             title: {
                                 display: true,
