<?php
// view-result-27.php (Öğrenci Rehberlik İhtiyacı Belirleme Anketi - RİBA Veli Sonuçları)
...
if (isset($_GET['id'])) {
    ...
    if (!$participantId) {
        ...
        try {
            ...
            if (!$participantData) {
                ...
                error_log("Participant not found for view-result-27 (ID: {$participantId}, Survey: {$surveyId})");
            } else {
                ...
                if ($dataSource == 'db' && !empty($participantData['admin_id']) && !empty($participantData['institution_logo_path'])) {
                   ...
                   else { error_log("Kurum logosu dosyası bulunamadı (view-result-27): " . $fullServerPath); }
                }
                ...
                if ($processedAnswerCount < $totalExpectedQuestionsFetched) {
                    ...
                    $evaluationText = "Bu anket sonuçları, çocuğunuzun rehberlik ihtiyacına yönelik veli algısını yansıtmaktadır. ";
                     ...
                     if ($validDimensionScoresExist) {
                         if ($highestDimension !== null && $highestAverage > 3) { // Ortalama 3'ün üzerindeyse anlamlı bir ihtiyaç algısı var diyebiliriz
                             $evaluationText .= "Sonuçlara göre, veli algısında en çok ihtiyaç duyulan alan '{$highestDimension}' olarak öne çıkmaktadır. ";
                             $evaluationText .= "Bu alana yönelik rehberlik çalışmalarının önceliklendirilmesi faydalı olabilir.";
                         } else {
            ...
            error_log("DB Error view-result-27 (ID: {$participantId}): " . $e->getMessage());
    ...
    if (isset($_SESSION['survey_result_data'], $_SESSION['survey_result_data']['survey_id']) && $_SESSION['survey_result_data']['survey_id'] == $surveyId) {
        ...
        if (isset($sessionData['answers'], $sessionData['parent_name']) && is_array($sessionData['answers'])) { // Veli anketine özel kontrol

            $participantData = [
                'name' => $sessionData['parent_name'], // Veli adı
                'child_name' => $sessionData['child_name'] ?? 'Belirtilmemiş', // Çocuk adı
                'child_class' => $sessionData['child_class'] ?? 'Belirtilmemiş', // Çocuk sınıfı
                'child_school_number' => $sessionData['child_school_number'] ?? 'Belirtilmemiş', // Çocuk okul no
                'child_gender' => $sessionData['child_gender'] ?? 'Belirtilmemiş', // Çocuk cinsiyeti
                'parent_relation' => $sessionData['parent_relation'] ?? 'Belirtilmemiş', // Veli yakınlık derecesi
            ...
            if ($processedAnswerCount < $totalExpectedQuestionsFetched) {
                 error_log("view-result-27 Session: Processed fewer valid answers ({$processedAnswerCount}) than expected ({$totalExpectedQuestionsFetched}) from session data.");
            ...
            error_log("Incomplete session data for survey 27: " . print_r($sessionData, true));
...
if ($psikometrikWebURL) {
    ...
    if (!file_exists($fullPsikoServerPath)) { $psikometrikWebURL = null; error_log("Psikometrik logo dosyası bulunamadı (view-result-27): " . $fullPsikometrikServerPath); }
}
...
<head>
    ...
    <style>
        ...
        @media print {
            ...
            .answers-table td:nth-child(4) { font-weight: normal; } /* Yazdırmada kalınlığı azalt */


            .chart-container {
                /* Yazdırmada grafiğin görünürlüğünü ve boyutını ayarlayın */
...
<div class="container">
    ...
    <?php else: // Katılımcı verisi var veya en azından boyut isimleri mevcut ?>
        ...
        <div class="participant-info">
             ...
             <?php if ($dataSource == 'session' && isset($participantData['child_name'])): ?>
                 <p><strong>Veli Ad Soyad:</strong> <?= htmlspecialchars($participantData['name']) ?></p>
                 <p><strong>Çocuk Adı:</strong> <?= htmlspecialchars($participantData['child_name']) ?></p>
                 <p><strong>Çocuk Sınıfı:</strong> <?= htmlspecialchars($participantData['child_class']) ?></p>
                 <p><strong>Çocuk Okul No:</strong> <?= htmlspecialchars($participantData['child_school_number']) ?></p>
                 <p><strong>Çocuk Cinsiyeti:</strong> <?= htmlspecialchars($participantData['child_gender']) ?></p>
                 <p><strong>Yakınlık Derecesi:</strong> <?= htmlspecialchars($participantData['parent_relation']) ?></p>
             <?php else: ?>
                 <?php // DB'den geliyorsa, bilgiler 'name' sütununda birleşik olabilir ?>
                 <p><strong>Veli/Çocuk Bilgileri:</strong> <?= htmlspecialchars($participantData['name']) ?></p>
                 <p><strong>Çocuk Sınıfı:</strong> <?= htmlspecialchars($participantData['class'] ?? 'Belirtilmemiş') ?></p>
             <?php endif; ?>
        ...
        <div class="chart-container">
             <h3></h3>
             <canvas id="ribaVeliChart"></canvas>
        </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const ctx = document.getElementById('ribaVeliChart');
