<?php
// take-survey-27.php (Öğrenci Rehberlik İhtiyacı Belirleme Anketi - RİBA Veli)
...
// --- Konfigürasyon ---
$surveyId = 27; // Anket ID'si
$testTitleDefault = "Öğrenci Rehberlik İhtiyacı Belirleme Anketi (RİBA) (VELİ)";
// ---------------------
...
// --- Sabit Veriler (Anket 27 - RİBA Veli) ---
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $error === null) {
    ...
    // Veli anketine özel alanlar
    $parentName = trim(htmlspecialchars($_POST['parent_name'] ?? '', ENT_QUOTES, 'UTF-8'));
    $childName = trim(htmlspecialchars($_POST['child_name'] ?? '', ENT_QUOTES, 'UTF-8'));
    $childClass = isset($_POST['child_class']) ? trim(htmlspecialchars($_POST['child_class'], ENT_QUOTES, 'UTF-8')) : null;
    $childSchoolNumber = isset($_POST['child_school_number']) ? trim(htmlspecialchars($_POST['child_school_number'], ENT_QUOTES, 'UTF-8')) : null;
    $childGender = isset($_POST['child_gender']) ? trim(htmlspecialchars($_POST['child_gender'], ENT_QUOTES, 'UTF-8')) : null;
    $parentRelation = isset($_POST['parent_relation']) ? trim(htmlspecialchars($_POST['parent_relation'], ENT_QUOTES, 'UTF-8')) : null;
    ...
    if ($post_error === null) { // Veritabanı hatası yoksa devam et
        ...
        } elseif (empty($parentName)) { // Veli Ad Soyad kontrolü
            $post_error = "Veli Ad Soyad alanı boş bırakılamaz.";
    ...
    if ($post_error === null) { // Tüm doğrulamalar başarılıysa devam et
        if ($adminId !== null) {
            ...
            try {
                ...
                // survey_participants tablosunda 'name' veli adı, 'class' çocuğun sınıfı/okulu gibi kullanılabilir.
                // Okul no, cinsiyet, yakınlık derecesi için ayrı sütunlar yoksa 'name' veya 'class' sütunlarına ek bilgi olarak kaydedilebilir
                // veya yeni sütunlar eklenmelidir. Mevcut şemaya uyum sağlamak için bilgileri birleştirelim.
                $participantNameForDB = $parentName . " (Çocuk: " . $childName . ", Sınıf: " . $childClass . ", Okul No: " . $childSchoolNumber . ", Cinsiyet: " . $childGender . ", Yakınlık: " . $parentRelation . ")"; // Bilgileri birleştirerek kaydetme örneği
                ...
                $stmtParticipant->bindParam(':name', $participantNameForDB, PDO::PARAM_STR);
                $stmtParticipant->bindParam(':class', $childClass, PDO::PARAM_STR); // Çocuğun sınıfını kaydetme örneği
                ...
                error_log("Admin Survey Submit Exception (S{$surveyId}, Admin{$adminId}, ParentName:{$parentName}): " . $e->getMessage());
                $post_error = "<b>Kayıt Sırasında Bir Hata Oluştu!</b> Lütfen tekrar deneyin veya yöneticiyle iletişime geçin. (Detaylar loglandı)";
            }

        } else {
            // **** SENARYO 2: ADMIN YOK -> KAYDETME, SONUÇ VERİSİNİ SESSION'A AT, view-result-27.php'ye YÖNLENDİR ****
            ...
            if ($post_error === null && !empty($questionIdToSortOrderMap)) { // questionIdToSortOrderMap POST doğrulamada çekildi
                ...
                $_SESSION['survey_result_data'] = [
                    ...
                    'parent_name' => $parentName, // Veli adı
                    'child_name' => $childName, // Çocuk adı
                    'child_class' => $childClass, // Çocuk sınıfı
                    'child_school_number' => $childSchoolNumber, // Çocuk okul no
                    'child_gender' => $childGender, // Çocuk cinsiyeti
                    'parent_relation' => $parentRelation, // Veli yakınlık derecesi
                    'timestamp' => time()
                ];
                session_write_close(); // Session yazmayı bitir
                // Yönlendirme yolu admin dizinini içerecek şekilde güncellendi
                header('Location: ../admin/view-result-27.php'); // view-result-27.php'ye yönlendir
                exit();
    ...
    <div class="container">
        ...
        <?php if (!$error && !empty($questions)): ?>
            ...
            <form method="POST" id="surveyForm" action="<?= $actionUrl ?>" novalidate>
                ...
                <div class="info">
                     <div class="info-grid">
                         <div>
                             ...
                             <input type="text" id="parent_name" name="parent_name" required placeholder="Adınızı ve soyadınızı girin..." value="<?= htmlspecialchars($form_data['parent_name'] ?? '') ?>" class="<?= ($post_error && empty($form_data['parent_name'])) ? 'validation-error' : '' ?>" aria-describedby="parentNameError">
                             <p id="parentNameError" class="validation-error-text" aria-live="polite"></p>
                         </div>
                         <div>
                             <label for="child_name">Çocuğunuzun Adı Soyadı:</label>
                             <input type="text" id="child_name" name="child_name" placeholder="Çocuğunuzun adını ve soyadını girin..." value="<?= htmlspecialchars($form_data['child_name'] ?? '') ?>" aria-describedby="childNameError">
                             <p id="childNameError" class="validation-error-text" aria-live="polite"></p>
                         </div>
                         <div>
                             <label for="child_class">Çocuğunuzun Sınıfı:</label>
                             <input type="text" id="child_class" name="child_class" placeholder="Örn: 10/A" value="<?= htmlspecialchars($form_data['child_class'] ?? '') ?>" aria-describedby="childClassError">
                             <p id="childClassError" class="validation-error-text" aria-live="polite"></p>
                         </div>
                         <div>
                             <label for="child_school_number">Çocuğunuzun Okul Numarası:</label>
                             <input type="text" id="child_school_number" name="child_school_number" placeholder="Okul numarasını girin..." value="<?= htmlspecialchars($form_data['child_school_number'] ?? '') ?>" aria-describedby="childSchoolNumberError">
                             <p id="childSchoolNumberError" class="validation-error-text" aria-live="polite"></p>
                         </div>
                         <div>
                             <label for="child_gender">Çocuğunuzun Cinsiyeti:</label>
                             <select id="child_gender" name="child_gender" aria-describedby="childGenderError">
                                 <option value="">Seçiniz</option>
                                 <option value="Erkek" <?= (isset($form_data['child_gender']) && $form_data['child_gender'] === 'Erkek') ? 'selected' : '' ?>>Erkek</option>
                                 <option value="Kız" <?= (isset($form_data['child_gender']) && $form_data['child_gender'] === 'Kız') ? 'selected' : '' ?>>Kız</option>
                             </select>
                              <p id="childGenderError" class="validation-error-text" aria-live="polite"></p>
                         </div>
                         <div>
                             <label for="parent_relation">Öğrenciye Yakınlık Dereceniz:</label>
                             <select id="parent_relation" name="parent_relation" aria-describedby="parentRelationError">
                                 <option value="">Seçiniz</option>
                                 <option value="Anne" <?= (isset($form_data['parent_relation']) && $form_data['parent_relation'] === 'Anne') ? 'selected' : '' ?>>Anne</option>
                                 <option value="Baba" <?= (isset($form_data['parent_relation']) && $form_data['parent_relation'] === 'Baba') ? 'selected' : '' ?>>Baba</option>
                                 <option value="Diğer" <?= (isset($form_data['parent_relation']) && $form_data['parent_relation'] === 'Diğer') ? 'selected' : '' ?>>Diğer</option>
                             </select>
                              <p id="parentRelationError" class="validation-error-text" aria-live="polite"></p>
                         </div>
                      </div>
                </div>
                ...
                <?php // Varsa, çocuğunuzun belirtmek istediğiniz diğer rehberlik ihtiyaçları alanı ?>
                <div class="info">
                    <label for="other_needs">Varsa, çocuğunuzun belirtmek istediğiniz diğer rehberlik ihtiyaçları:</label>
        ...
        // Eksik varsa alert gösterir ve formu GÖNDERMEZ.
         ...
         if (form) {
             form.addEventListener('submit', function(event) {
                 ...
                 // Veli Ad Soyad kontrolü (Zorunlu)
                 const parentNameInput = document.getElementById('parent_name');
                 const parentNameErrorP = document.getElementById('parentNameError');
                 if (parentNameInput && parentNameErrorP) {
                     if (!parentNameInput.value.trim()) {
                         parentNameInput.classList.add('validation-error');
                         parentNameErrorP.textContent = 'Lütfen veli adınızı ve soyadınızı girin.';
                         isValid = false;
                         if (!firstErrorElement) firstErrorElement = parentNameInput;
                     } else {
                         parentNameInput.classList.remove('validation-error');
                         parentNameErrorP.textContent = '';
                     }
                 }

                 // Diğer veli/çocuk bilgileri alanları (Zorunlu değilse sadece stil kaldırılır)
                 // Eğer zorunlu ise benzer kontrol blokları eklenmelidir.
                 const optionalFields = ['child_name', 'child_class', 'child_school_number', 'child_gender', 'parent_relation'];
                 optionalFields.forEach(fieldId => {
                     const input = document.getElementById(fieldId);
                     const errorP = document.getElementById(fieldId + 'Error');
                     if (input && errorP) {
                          // Şu an için zorunlu değiller, sadece hata stilini kaldır
