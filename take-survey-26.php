<?php
// take-survey-26.php (Şiddet Algısı Anketi - Veli)
...
// --- Konfigürasyon ---
$surveyId = 26; // Anket ID'si
$testTitleDefault = "Şiddet Algısı Anketi (VELİ)";
// ---------------------
...
// --- Sabit Veriler (Anket 26 - Şiddet Algısı Veli) ---
// Cevap Seçenekleri - Bunlar doğrudan kaydedilecek metinlerdir.
$optionsList = ["Şiddettir", "Kararsızım", "Şiddet Değildir"];
// Bu anketin puanlama veya alt ölçek mantığı yoktur, bu nedenle ilgili haritalar kaldırılmıştır.
    ...
    if ($post_error === null) { // Tüm doğrulamalar başarılıysa devam et
        if ($adminId !== null) {
            ...
            try {
                ...
                // Okul no için ayrı bir sütun yoksa 'class' sütununa ek bilgi olarak kaydedilebilir veya yeni sütun eklenmelidir.
                ...
                $participantNameForDB = $parentName . " (Çocuk: " . $childName . ", Sınıf: " . $childClass . ", Okul No: " . $childSchoolNumber . ")"; // Bilgileri birleştirerek kaydetme örneği
                ...
                foreach ($answers as $actual_db_id => $answer_text) { // $answer_text artık "Şiddettir", "Kararsızım", "Şiddet Değildir"
            ...
            // **** SENARYO 2: ADMIN YOK -> KAYDETME, SONUÇ VERİSİNİ SESSION'A AT, view-result-26.php'ye YÖNLENDİR ****
            ...
            if ($post_error === null && !empty($questionIdToSortOrderMap)) { // questionIdToSortOrderMap POST doğrulamada çekildi
                ...
                foreach ($answers as $qDbId => $answerValue) { // $answerValue artık "Şiddettir", "Kararsızım", "Şiddet Değildir"
                ...
                header('Location: ../admin/view-result-26.php'); // view-result-26.php'ye yönlendir
                exit();
    ...
    <div class="container">
        ...
        <?php if (!$error && !empty($questions)): ?>
            ...
            <div class="instructions">
                ...
                <p>Aşağıda bazı durumlar verilmiştir. Sizden aşağıda yer alan olayları şiddet olarak görüp görmediğinizi belirtmeniz beklenmektedir. Olayların karşısında yer alan "Şiddettir", "Kararsızım", "Şiddet Değildir" ifadelerinden size uygun seçeneğin altındaki parantezin içine (X) işareti koyunuz.</p>
        ...
        // Eksik varsa alert gösterir ve formu GÖNDERMEZ.
         ...
         if (form) {
             form.addEventListener('submit', function(event) {
                 ...
                 // Diğer veli/çocuk bilgileri alanları zorunlu değilse kontrol eklemeye gerek yok.
