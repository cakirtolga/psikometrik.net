...
// Assuming config.php is in the 'src' directory, which is in the same directory as this survey file.
require_once __DIR__ . '/src/config.php';

// Define the survey ID for this specific file (Bu dosya için anket ID'si belirlenir)
$surveyId = 8; // Anket ID'si 8 olarak ayarlandı (Benlik Tasarımı Envanteri)
    ...
    try {
        ...
        if ($admin) {
            $adminId = $admin['id']; // Admin bulundu, ID'yi kullanabiliriz
             error_log("Admin ID validated from URL for Survey 8: " . $adminId);
        } else {
            $error = 'Geçersiz veya bulunamayan yönetici ID\'si.';
            error_log($error . " - Attempted ID for Survey 8: " . $potentialAdminId);
             // adminId null kalır
        }
    } catch (PDOException $e) {
        $error = 'Yönetici bilgisi alınırken veritabanı hatası: ' . $e->getMessage();
        error_log("Admin ID validation PDO Exception for Survey 8: " . $e->getMessage());
         // adminId null kalır
    }
} else {
    $error = 'Yönetici ID\'si eksik veya geçersiz.';
    error_log($error . " - Received admin_id for Survey 8: " . ($_GET['admin_id'] ?? 'Not set'));
     // adminId null kalır
}
...
// Fetch survey information for Survey ID 8 (Anket bilgileri çekilir)
...
// Fetch questions for Survey ID 8 ordered by sort_order (Anketin soruları çekilir)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    error_log("--- POST request received for Survey 8 ---");
    ...
    error_log("POST Data Received for Survey 8: " . print_r($_POST, true));
    error_log("Name: " . $name);
    ...
    if (empty($name) || empty($class)) {
        ...
        error_log("Validation failed for Survey 8: Name or Class is empty.");
    }
    ...
    // This ensures all questions across all pages have been answered
         ...
         error_log("Validation failed for Survey 8: Incorrect number of answers received. Expected: " . $totalQuestions . ", Received: " . count($answers));
         ...
         if (!empty($missingQuestionIds)) {
             error_log("Missing question IDs for Survey 8: " . implode(', ', $missingQuestionIds));
         }
    ...
    // If validation passes, proceed with database insertion (Doğrulama başarılıysa veritabanına kayıt yapılır)
        ...
        if (is_null($adminId)) {
             ...
             error_log("Insertion blocked for Survey 8: Admin ID is null.");
        } else {
            try {
                // Start a transaction for atomic database operations (Veritabanı işlemleri için transaction başlatılır)
                $pdo->beginTransaction();
                error_log("Transaction started for Survey 8.");

                // --- Insert Participant --- (Katılımcı kaydı yapılır)
                error_log("Attempting to insert participant for Survey 8: Name=" . $name . ", Class=" . $class . ", SurveyId=" . $surveyId . ", AdminId=" . $adminId);
                ...
                if ($participantInsertSuccess) {
                     error_log("Participant inserted successfully for Survey 8 with ID: " . $participantId);
                } else {
                     error_log("Participant insert failed for Survey 8. PDO ErrorInfo: " . print_r($stmt->errorInfo(), true));
                ...
                error_log("Attempting to insert answers for Survey 8, participant ID: " . $participantId);
                ...
                foreach ($answers as $qid => $answer) {
                    ...
                    if (is_numeric($qid) && is_string($answer)) {
                        ...
                        if ($answerInsertSuccess) {
                             error_log("Inserted answer for Survey 8, QID " . $qid . ": " . $answer);
                        } else {
                             error_log("Answer insert failed for Survey 8, QID " . $qid . ". PDO ErrorInfo: " . print_r($stmt->errorInfo(), true));
                        ...
                        error_log("Skipping invalid answer data for Survey 8: QID=" . $qid . ", Answer=" . $answer);
                ...
                if ($participantInsertSuccess && $allAnswersInserted) { // Katılımcı eklendiyse VE tüm cevaplar başarılıysa commit yapılır
                     $pdo->commit();
                     error_log("Transaction committed for Survey 8. Redirecting to tamamlandi.php");
                     ...
                     error_log("Transaction rolled back for Survey 8 due to insertion failure.");
                }
                ...
                error_log("PDO Exception for Survey 8: " . $e->getMessage());
                ...
                // $error = "Anket kaydedilirken bir hata oluştu.";
                 ...
                 error_log("General Exception for Survey 8: " . $e->getMessage()); // Hata loglanır
            }
        }
    }
...
<body class="bg-[#f0fdf4] min-h-screen p-4">
                        ...
                        ?>
                            <div class="mb-8 question">
                                ...
                                <div class="flex gap-3 mb-6 options">
                                    <?php
                                    // Use the specific options for Survey 8: 'Tam benim gibi', 'Biraz benim gibi', 'Kararsızım', 'Hayır', 'Asla'
                                    $options = ['Tam benim gibi', 'Biraz benim gibi', 'Kararsızım', 'Hayır', 'Asla'];
    ...
    <script>
        ...
        console.log('Total question groups for Survey 8:', totalGroups); // Log total groups

        // Function to show a specific page (question group)
        function showPage(index) {
            console.log('Attempting to show group for Survey 8:', index, 'Current group:', current, 'Total groups:', totalGroups); // Log attempt

            // Bounds checking for index
            if (index < 0 || index >= totalGroups) {
                console.error('Invalid page index for Survey 8:', index);
                return false; // Prevent page change
            }

            const isLastPage = index === totalGroups - 1;
            console.log('Is last page for Survey 8:', isLastPage); // Log if it's the last page
            ...
            current = index;
            console.log('Page changed for Survey 8 to:', current); // Log successful page change
             ...
             if (groups[current]) { // Check if the group exists
                 groups[current].querySelectorAll('.question-button').forEach(btn => {
                     ...
                     if (qidMatch && qidMatch[1]) {
                         ...
                         console.error('Could not extract question ID from onclick attribute for button in Survey 8:', btn);
                     }
                 });
             }
