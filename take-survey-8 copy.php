...
$surveyId = 8; // Benlik Tasarımı Envanteri id'si
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$name || !$class) {
        $error = "Lütfen adınızı ve sınıfınızı girin.";
    } else {
        // Katılımcıyı kaydet
  ...
  <div class="bg-white p-8 rounded shadow-md w-full max-w-4xl">
    ...
    <form method="POST" class="space-y-6">
      ...
      <?php foreach ($questions as $question): ?>
        <div class="mb-6">
          <label class="block mb-2 font-semibold"><?php echo htmlspecialchars($question['question']); ?></label>
          <div class="flex flex-col gap-2 mt-2">
              <label><input type="radio" name="answers[<?php echo $question['id']; ?>]" value="Tam benim gibi" required> Tam benim gibi</label>
              <label><input type="radio" name="answers[<?php echo $question['id']; ?>]" value="Biraz benim gibi" required> Biraz benim gibi</label>
              <label><input type="radio" name="answers[<?php echo $question['id']; ?>]" value="Kararsızım" required> Kararsızım</label>
              <label><input type="radio" name="answers[<?php echo $question['id']; ?>]" value="Hayır" required> Hayır</label>
              <label><input type="radio" name="answers[<?php echo $question['id']; ?>]" value="Asla" required> Asla</label>
          </div>
        </div>
      <?php endforeach; ?>
